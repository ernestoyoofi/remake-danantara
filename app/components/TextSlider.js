...
export default function TextSlider({ listString }) {
  ...
  return <div className="w-full mt-3">
    <div className="relative flex items-center justify-between w-full mx-auto text-center">
      <button className="flex items-center justify-center w-[22px] h-[190px]" onClick={prevSlide}>
        <HiChevronLeft size={28}/>
      </button>
      <div className="relative overflow-hidden flex-1 mx-4 w-[calc(100%-44px)] h-[190px]">
        <div className="flex transition-transform duration-300 ease-in-out" style={{ transform: `translateX(-${currentIndex * 100}%)` }}>
          {listText.map((text, index) => (
            <div key={index} className="flex items-start justify-center flex-shrink-0 w-full h-[190px]">
              <div className="text-left">{text}</div>
            </div>
          ))}
        </div>
      </div>
      <button className="flex items-center justify-center w-[22px] h-[190px]" onClick={nextSlide}>
